...
def replace_digits(s, symbols: str = " ", only_blocks=True):
    """
...
def remove_digits(s, only_blocks=True):
    """
...
def replace_punctuation(s, symbol: str = " "):
    """
    ...
    return s.str.replace(rf"([{string.punctuation}])+", symbol, regex=True)
...
def replace_stopwords(s, symbol: str, stopwords: Optional[Set[str]] = None):
    """
    ...
    if stopwords is None:
        import researcher.representations.stopwords as _stopwords

        stopwords = _stopwords.DEFAULT
    return s.apply(_replace_stopwords, args=(stopwords, symbol))


def remove_stopwords(s, stopwords: Optional[Set[str]] = None, remove_str_numbers=False):
    """
...
# def phrases(
#     s, min_count: int = 5, threshold: int = 10, symbol: str = "_"
# ) :
...
def replace_hashtags(s, symbol: str):
...
def build_lemma(s):
    """Lemmatize the given Pandas Series."""
    # apply WordNetLemmatizer to the series
...
def build_stem(s):
    """Stem the given Pandas Series."""
    # apply PorterStemmer to the series
    stemmer = PorterStemmer()
    stems = s.apply(lambda x: [stemmer.stem(word) for word in word_tokenize(x)])
    return stems


def build_clause(s, custom_clause_word=None):
    """Split the given Pandas Series into clauses."""
    clause_reg = r"[\.\!\\\/\|,\?\;\:_\-=+]"
    clause_words = (
    ...
    # split the series into clauses
    clauses = s.str.split(clause_sep, flags=re.IGNORECASE)
    return clauses
